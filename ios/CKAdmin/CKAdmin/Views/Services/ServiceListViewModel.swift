...
/// View model for the service list screen.
///
/// Manages paginated loading, status filtering, and loading/error states.
...
final class ServiceListViewModel {
    ...
    /// The loaded service records.
    private(set) var services: [ServiceListItem] = []
    ...
    var selectedStatus: ServiceStatusFilter = .all {
        didSet {
    ...
    /// - Parameter apiClient: The client used to fetch service data.
    ...
    /// Loads the first page of services. Resets existing data.
    @MainActor
    ...
    /// Loads the next page of services if available.
    ///
    ...
    func loadNextPage() async {
        ...
        do {
            ...
            let response = try await fetchServices(page: nextPage)
            services.append(contentsOf: response.data)
    ...
    /// - Returns: The paginated response with service data and metadata.
    private func fetchServices(page: Int) async throws -> PaginatedResponse<ServiceListItem> {
        ...
        let endpoint = Endpoint(path: "/admin/services", queryItems: queryItems)
