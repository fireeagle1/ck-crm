<x-admin-layout>
    <x-slot:title>Edit Decision</x-slot:title>

    <div class="flex items-center justify-between mb-4">
        <h1 class="text-2xl font-bold">Edit Decision</h1>
    ...
    <div class="bg-white rounded-lg border p-6 max-w-3xl">
        <form method="POST" action="{{ route('admin.projects.decisions.update', [$project, $decision]) }}">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-5">
                ...
                <div>
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', $decision->title) }}" required
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="4"
                              class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500"
                              placeholder="Optional details about this decision">{{ old('description', $decision->description) }}</textarea>
                ...
                <div>
                    ...
                    <select name="category" id="category"
                        ...
                        @foreach (\App\Models\ProjectDecision::CATEGORIES as $category)
                            <option value="{{ $category }}" {{ old('category', $decision->category) === $category ? 'selected' : '' }}>
                                {{ $category }}
                            </option>
                        @endforeach
                    </select>
