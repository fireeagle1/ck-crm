        ...
        <form method="POST" action="{{ route('admin.services.update', $service) }}">
            ...
            <div class="space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label for="company_id" class="block text-sm font-semibold text-gray-700">Customer</label>
                    ...
                    <div>
                        ...
                        <select name="service_type" id="service_type" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500">
                            <option value="">—</option>
                            @foreach (['Web Hosting', 'Technical Support', 'Other'] as $type)
                ...
                <div>
                    <label for="service_short" class="block text-sm font-semibold text-gray-700">Service Name</label>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label for="domain_name" class="block text-sm font-semibold text-gray-700">Domain Name</label>
                    ...
                    <div>
                        <label for="cpanel_username" class="block text-sm font-semibold text-gray-700">cPanel Username</label>
                ...
                @if ($service->stripe_subscription_id)
                    <div class="border border-green-200 rounded-md p-4 bg-green-50">
                        <p class="text-sm font-semibold text-green-900 mb-1">Pricing managed by Stripe</p>
                        <p class="text-xs text-green-700">
                            Subscription: <span class="font-mono">{{ $service->stripe_subscription_id }}</span><br>
                            Pricing is synced automatically. To change pricing, update the subscription in Stripe.
                        </p>
                ...
                <div class="flex gap-3 pt-2">
                    <button type="submit" class="px-5 py-2.5 bg-blue-600 text-white rounded-md text-sm font-semibold hover:bg-blue-700">Save Changes</button>
                    <a href="{{ route('admin.services.show', $service) }}" class="px-5 py-2.5 border rounded-md text-sm font-semibold hover:bg-gray-50">Cancel</a>
                </div>
            </div>
        </form>
    </div>
